...
def main():
    ...
    parser.add_option("--dir", dest="data_directory", type=str, help="get the data directory",
                      default="/home/nw/quanox/soft/supervised-lda/mytest/data20newsgroup")
    ...
    parser.add_option("--vocab", dest="vocabulary_list_file", type=str, help="get the vocabulary list file name",
                      default="vocab.word_id.dict")
    ...
    get_map(vocab_list_file=options.data_directory + "/" + options.vocabulary_list_file, word_dict=word_dict)
    get_topic_stats(word_topic_count_file=options.data_directory + "/" + options.word_topic_count_file, topic_stat=topic_stat,
                    word_topic_table=word_topic_table)
    normalize_probablities(word_topic_table_file=options.data_directory + "/" + options.word_topic_table_file, topic_stat=topic_stat,
                           word_dict=word_dict, word_topic_table=word_topic_table,
                           readable_word_topic_file= options.data_directory + "/" + options.readable_word_topic_file)
    doc_topic_distribution(readable_doc_topic_file= options.data_directory + "/" + options.readable_doc_topic_file,
                           doc_topic_table_file= options.data_directory + "/" + options.doc_topic_table_file)
